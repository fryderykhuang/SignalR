...
namespace Microsoft.AspNet.SignalR.Transports
{
    public class LongPollingTransport : ForeverTransport, ITransport
    {
        ...
        public LongPollingTransport(HttpContext context, IDependencyResolver resolver)
            : this(context,
        ...
        protected override bool SuppressReconnect
        {
            get
            {
                return !Context.Request.Path.Value.EndsWith("/reconnect", StringComparison.OrdinalIgnoreCase);
            }
        }
        ...
        protected override Task InitializeMessageId()
        {
            ...
            {
                return Context.Request.ReadFormAsync().Then((form, t) =>
                {
        ...
        {
            string groupsToken = Context.Request.Query["groupsToken"];

            if (groupsToken == null)
            {
                return Context.Request.ReadFormAsync().Then(form => (string) form["groupsToken"]);
            }
        ...
        {
            IFormCollection form = await Context.Request.ReadFormAsync().PreserveCulture();
        ...
        private static Task PerformKeepAlive(object state)
        {
            ...
            transport.Context.Response.Body.Write(_keepAlive.Array, _keepAlive.Offset, _keepAlive.Count);
