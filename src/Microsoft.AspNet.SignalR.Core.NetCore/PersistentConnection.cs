...
//            var response = new OwinResponse(environment);
        ...
        /// </exception>
        public virtual Task ProcessRequestCore(HttpContext context)
        {
            if (context == null)
            {
        ...
        private Task ProcessRequestPostGroupRead(HttpContext context, string groupsToken)
        {
...
#else
            return Transport.ProcessRequest(connection).OrEmpty().Catch(Trace);
#endif
        }
        ...
        {
            return request.Path.Value.EndsWith("/negotiate", StringComparison.OrdinalIgnoreCase);
        }

        private static bool IsStartRequest(HttpRequest request)
        {
            return request.Path.Value.EndsWith("/start", StringComparison.OrdinalIgnoreCase);
        }

        private static bool IsPingRequest(HttpRequest request)
        {
            return request.Path.Value.EndsWith("/ping", StringComparison.OrdinalIgnoreCase);
        }
